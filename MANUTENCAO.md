...
1.  **Modelos (`backend/app/models/`)**: Representam as tabelas do banco de dados e seus relacionamentos (SQLAlchemy).
2.  **Esquemas (`backend/app/schemas/`)**: Validação de entrada e saída de dados (Pydantic).
3.  **Serviços (`backend/app/services/`)**: Contém a "Regra de Negócio". É onde a lógica pesada acontece.
4.  **Roteadores (`backend/app/routers/`)**: Define os pontos de entrada (Endpoints) da API e gerencia as requisições HTTP.
5.  **Banco de Dados (`backend/app/database/`)**: Configuração da conexão e gerenciamento de sessões.
...
1. Altere o modelo em `backend/app/models/`.
2. Atualize o esquema em `backend/app/schemas/`.
3. Verifique se o serviço em `backend/app/services/` precisa lidar com esse novo campo.
